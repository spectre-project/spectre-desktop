...
class SpectreProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['spectred','mqtt','mysql','pgsql', 'txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['spectred','mqtt','mysql','pgsql','simulator','txgen'];
		//const typeMap = { };
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.spectre-desktop','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, spectre } = pkg;
			return {
				folder, name, description, main, engines, ...spectre
			}
		});
	}
...
module.exports = SpectreProcessManager;
