...
const {Client:SpectreGRPC} = require('@spectre/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Spectred extends Daemon {
	...
	start() {
		this.log("spectred::start() invoked...");
		return new Promise(async (resolve,reject) => {

			if(this.task.conf['reset-peers'])
				this.cleanupResidualData();

			this.log("spectred::start() running");
			...
			if (!("uacomment" in args)) {
				const uacomment = `spectre-desktop_${pkg['version']}`
				this.writeToSink(`SPECTRED: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`SPECTRED: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`SPECTRED: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`SPECTRED: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`SPECTRED: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'spectred.conf');
			...
			await this.VSARGS([this.getDeployableBinary('spectred'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('spectred')
					];
			...
			resolve();
			// mutes all spectred processes by default
			this.mute = true;
		})
	}
	...
	async stop() {
		...
		if(this.rpc) {
			...
			try {
				if(!process.env.SPECTRE_JSON_RPC)				
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('SPECTRED monitor:'.bold,ex);
	            return reject(ex);
	        }
