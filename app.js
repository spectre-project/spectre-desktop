...
class App extends FlowApp{
	...
	async initConfig(){
		...
		if(reset) {
			try {
				...
				alert('Error resetting Spectre Desktop data folder: '+ex);
			}
	...
	*/
	dataDirInitError(){
		console.log(`Please start app with --init=/path/to/data/dir or --init for default (~/.spectre-desktop/data)`);
		this.exit();
	}
	...
	*/
	...
	async removeDataDir(){
		try {
			let datadir2path = path.join(this.dataFolder, "spectred-sd0", "spectre-mainnet", "datadir2");
	...
	*/
	...
	setSkipUTXOIndex(skipUTXOIndex){
		...
		return this.setModuleArgs("spectred:", {}, {"skip-utxoindex":skipUTXOIndex})
	}
	getSkipUTXOIndex(){
		let {args, params} = this.getModuleArgs("spectred:");
	...
	setUpnpIfMissing(upnpEnabled) {
		if (this.config?.modules) {
			Object.keys(this.config.modules).forEach((k) =>{
				if (k.startsWith('spectred:') && !('upnpEnabled' in this.config.modules[k])) {
